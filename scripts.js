...
const flipButton = document.getElementById('flip-image')
flipButton.onclick = () => {
    
    //desestruturando altura e largura
    const { width: imgW, height: imgH } = image
    const { width: previewW, height: previewH } = photoPreview
    ...
    //passando imagem espelhada para o html
    const flipedImage = ctx.getImageData(0, 0, canvas.width, canvas.height)
    ctx.putImageData(flipedImage, 0, 0)
    photoPreview.src = canvas.toDataURL()
